...
(*let rec print_val = function 
  ...
  | InF(e,l,env) -> ("Ferm avec env:("^(print_env env)^")")
  | InFR(n,e,l,env) -> ("FermRec "^n^" avec env:("^(print_env env)^")")

and print_env = function [] -> "" | (x,y)::q -> ("("^x^(print_val y)^")"^(print_env q))
*)
let rec print_list = function 
[] -> ()
| (x,_)::l ->  Printf.printf "%s\t" x ;print_list l
...
let rec inEnv x env =
  match env with
  [] -> ( 
    Printf.printf "l'id %s n'existe pas dans [" x;
    print_list env;
    Printf.printf " ]";
    failwith "pas dans l'env"
  )
      ...
      let eval_e = eval_expr e env in
        match eval_e with
          ...
          |InFR(nom,e,param,env_) -> ((
            eval_expr e (List.append (List.combine param vals_list)  ((nom,InFR(nom,e,param,env_))::env_ ) )
          ))
...
let l1 = [1;3;4] in
let l2 = ["a";"b";"c"] in
print_list (List.remove_assoc "b" (List.combine l2 l1))
*)
